...
class PerguntaViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for a in 0 ... 2{
            ...
            questao.setPergunta(perguntasTeste[a])
            for _ in 0 ... 2{
        ...
        resposta2.setTitle(respostasTeste[1].resposta, for: [])
        resposta3.setTitle(respostasTeste[2].resposta, for: [])    
        // Do any additional setup after loading the view.
    }
    
    @IBAction func resposta1Action(_ sender: UIButton) {
        if respostasTeste[0].indice == 0{
            
            qtdAcertos += 1
        }
        sleep(1)
        resposta1.setTitle(respostasTeste[3].resposta, for: [])
    }
